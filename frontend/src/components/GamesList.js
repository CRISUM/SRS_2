...
const GamesList = () => {
  ...
  useEffect(() => {
    if (currentPage === 1) {
      fetchGames(1, limit, searchTerm, selectedTags, cloudGamingOnly, steamDeckOnly);
    } else {
      setCurrentPage(1);
    }
  }, [selectedTags, cloudGamingOnly, steamDeckOnly]);
...
// 更新 fetchGames 函数以接收标签和 Steam Deck 参数
const fetchGames = async (page, pageLimit, search, tags = [], cloudOnly = false, steamDeckOnly = false) => {
  setLoading(true);
  setError(null);
  setIsSearching(!!search || tags.length > 0 || cloudOnly || steamDeckOnly);
  ...
  try {
    ...
    // Add Steam Deck filter if selected
    if (steamDeckOnly) {
      params.steam_deck = true;
    }
...
return (
  <div className="games-list">
        ...
        <div className="flex flex-wrap gap-2 items-center">
          ...
          <div className="flex items-center">
            <input
              ...
              onChange={() => setSteamDeckOnly(!steamDeckOnly)}
              className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
            />
            <label htmlFor="steam-deck" className="ml-2 text-sm text-gray-700 flex items-center">
              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" className="mr-1" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          {(selectedTags.length > 0 || cloudGamingOnly || steamDeckOnly || searchTerm) && (
            <button
    ...
    {/* Use the new TagFilter component */}
    <TagFilter
    ...
    {loading ? (
      ...
      <>
        ...
        {(selectedTags.length > 0 || cloudGamingOnly || steamDeckOnly) && (
          <div className="mb-4 flex flex-wrap gap-2 items-center">
            ...
            {steamDeckOnly && (
              <span className="bg-purple-100 text-purple-800 text-xs px-2 py-1 rounded-full flex items-center">
                Steam Deck
                <button
                  onClick={() => setSteamDeckOnly(false)}
